...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
	    ...
	    int[] resultarr = new int[numVertices];

	    for(int i = 0; i < this.numVertices; i++) {
	        int numOut = this.getNeighbors(i).size();
	        int numIn = this.getInNeighbors(i).size();

	        resultarr[i] = numOut + numIn;
	    }
	    
	    List<Integer> resultList = new ArrayList<Integer>();
	    Collections.addAll(resultList, Arrays.stream(resultarr).boxed().toArray(Integer[]::new));
	...
	public static void main (String[] args) {
		GraphLoader.createIntersectionsFile("data/maps/PacHeights.map", "data/intersections/myucsd.intersections");
